package submit;
//sever side er jonno worker class
import java.io.*;
...
//now listing own files
            List<String> associatedList = server.privatelyUploadedFiles.get(username);
            List<String> associatedList2 = server.publiclyUploadedFiles.get(username);
            if (associatedList != null|| associatedList2!=null)
                writer.println("User: " + username+"(You)");
            ...
            if (associatedList2 != null) {
                writer.println("Own publicly uploaded files " + ": " + associatedList2);}
...
//        writer.println("File upload completed");
        ...
        if(sum<sz) allOkay=false;  //sum jodi less than size hoy then discard kore dibe. logout hoye geleo sum less than size hobe
        if(allOkay==true) {
    ...
    private void handleFileDownload() throws IOException {
        ...
        String whoseFile=reader.readLine();
        String whichFile= reader.readLine();
        if( ( server.publiclyUploadedFiles.get(whoseFile)!=null && server.publiclyUploadedFiles.get(whoseFile).contains(whichFile) )|| (whoseFile.equalsIgnoreCase(this.username)&& server.privatelyUploadedFiles.get(whoseFile)!=null && server.privatelyUploadedFiles.get(whoseFile).contains(whichFile)))
        {
            if (file.exists()) {
