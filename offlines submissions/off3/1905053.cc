...
{
  float tm= Simulator::Now ().GetSeconds ();
  //cwdFile1 << tm << "\t" << newCwnd<< std::endl;
...
int main(int argc, char *argv[]) {
    ...
    double packet_loss_rate = (1.0 / std::pow(10, packet_loss_exp)); //10^-6
     double bandwidth_delay_product = bottleneckDelay*bottleneckDataRate/packSz;
    ...
    for(int i=0;i<nFlows; i++)
    {
     ...
     //====================  Trace Connection for Congestion Window ======================
      ...
      ns3TcpSocket->TraceConnectWithoutContext ("CongestionWindow", MakeBoundCallback (&CongestionWindowTracer, stream));
       //ns3TcpSocket->TraceConnectWithoutContext ("CongestionWindow", MakeBoundCallback (&CongestionWindowTracer, i));
    }
 ...
 std::ofstream cwdFile1("GeneratedFiles/"+folder_name+"/CongestionWindowVSTime1.dat", std::ios::app);  
   ...
   //============================Throughput Calculation =======================================
    ...
    //Fairness measures or metrics are used in network engineering to determine whether users or applications are receiving
    // a fair share of system resources. There are several mathematical and conceptual definitions of fairness

   // jain index= (sum of xi)^2/(n * sum of xi^2) ;                  where Xi is throughput, nis number of flows.. here n=4;
