...
string convertToBinary(int num)
{
    ...
    while(num!=0)
    {
        binaryString.push_back((num % 2) + '0');
        num=num/2;
    }
...
void correctErrors(string &data) 
{
    ...
    vector<int> errorPositions; // Stores positions of error bits
    ...
    while (dataIndex < n) 
    {
        ...
        {
            errorPositions.push_back(dataIndex); // Position of a data bit
        }
        dataIndex++;
    }
    ...
    {
        errorBitPosition += parityValues[i] * (1 << i);
    }
