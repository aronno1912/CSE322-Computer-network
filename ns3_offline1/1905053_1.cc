...
// sender ra hocche ashole client
using namespace ns3;
...
main(int argc, char* argv[])
{
   ...
   // wifi.SetRemoteStationManager ("ns3::AarfWifiManager");
    ...
    // for(double i=0;i<nSender;i++)
    // {
    //     nodePositionList1->Add(Vector(0.0, i*3.0, 0.0));
    // }
    // nodePositionList1->Add(Vector(3.0, ((nNodes-1)*3.0)/2.0, 0.0)); // AP
    // mobility.SetPositionAllocator(nodePositionList1);
  
    // mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    // mobility.Install(wifiStaNodes);
    // mobility.Install(wifiApNode);
    // Ptr<ListPositionAllocator> nodePositionList2 = CreateObject<ListPositionAllocator>();
    // for(double i=0;i<nReceiver;i++){
    //     nodePositionList2->Add(Vector(9.0, i*3.0, 0.0));
    // }
    // nodePositionList2->Add(Vector(6.0, ((nNodes-1)*3.0)/2.0, 0.0)); // AP
    // mobility.SetPositionAllocator(nodePositionList2);
   ...
   // =======================Sender Application Setup:=====================================================
    ...
    // udp->TraceConnectWithoutContext("Tx", MakeCallback(&traceTx));
